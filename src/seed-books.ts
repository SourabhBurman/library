...
export const seedBooks = async () => {
  ...
  const adjectives = ["The Great", "Secrets of", "Mystery of", "Return of", "Journey to", "Advanced", "Mastering"];
  const nouns = ["Mountains", "Code", "Space", "Oceans", "Time", "Magic", "History"];
  ...
  for (let i = 1; i <= 50; i++) {
    ...
    books.push({
      ...
      cost: price,
      rent_price: Math.floor(price * 0.1), // Rent is 10% of cost
      published_date: new Date(new Date().getTime() - Math.random() * 100000000000), 
    });
  }
  ...
  try {
    ...
    console.log(`✅ Successfully seeded ${books.length} dummy books into the database!`);
  } catch (error) {
