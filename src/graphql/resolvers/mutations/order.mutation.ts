...
import { ORDER_STATUS, ORDER_STATUS_ENUM, TRANSACTION_TYPE } from "../../../enums";
  ...
  placeOrder: async (
    ...
    try {
      ...
      const GET_AMOUNT = {
        ...
        [TRANSACTION_TYPE.BORROW]: bookDetail.rent_price,
      }
      ...
      if(transactionType === TRANSACTION_TYPE.BORROW){
       ...
       if(bookDetail?.quantity_available <= 0){
      ...
      const newOrder = queryRunner.manager.create(Order, {
        ...
        user: currentUser,
        library_book: libraryBookDetail,
      })
      ...
      await queryRunner.manager.update(LibraryBook, libraryBookDetail.id, {
        quantity_available: libraryBookDetail.quantity_available - 1,
      });
      ...
      const newTransaction = queryRunner.manager.create(TransactionHistory, {
         order: newOrder,
         transaction_type: transactionType as TRANSACTION_TYPE,
      });
  ...
  returnOrder: async (
    ...
    try {
      ...
      const orderDetail = await queryRunner.manager.findOne(Order, {
        where: { id: orderId },
        relations: ["library_book", "library_book.book", "library_book.library", "user"],
      });

      const libraryBookDetail = orderDetail?.library_book;
      ...
      if (orderDetail.expectedReturnDate && today.getTime() > orderDetail.expectedReturnDate.getTime()) {
        const diffTime = today.getTime() - orderDetail.expectedReturnDate.getTime();
        extraDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
      }
      
      const penalty = extraDays * (PENALTY_PERCENT * (bookDetail.rent_price || 0));
      ...
      const savedOrder = await queryRunner.manager.update(Order, orderDetail.id,{
